...
| 🏛️ **Standards & Compliance Guide** | MITRE ATLAS threat modeling, OWASP Top 10 for LLMs, the OWASP GenAI Red Teaming Guide (four-phase methodology), the **Q2 2026 GenAI Security Solutions Landscapes** (incl. the first-ever Red Teaming Landscape), NIST AI RMF, ISO 42001, and EU AI Act playbooks. | **[STANDARDS_AND_COMPLIANCE.md](STANDARDS_AND_COMPLIANCE.md)** |
| 📊 **Benchmarks & Datasets Index** | Standardized safety evaluation frameworks (HarmBench, AdvGLUE, CyberSecEval) plus the **agentic-eval set** — AgentDojo, InjecAgent, AgentHarm, and **Agent Security Bench (ASB)** (attacks *and* defenses across the full agent lifecycle) — and adversarial datasets. | **[BENCHMARKS_AND_DATASETS.md](BENCHMARKS_AND_DATASETS.md)** |
...
| 🚨 **Incidents & Guidance (2025–26)** | Disclosed real-world incidents — EchoLeak (CVE-2025-32711), the MCP breach cluster (now with named CVEs: MCP Inspector RCE **CVE-2025-49596**, mcp-remote RCE **CVE-2025-6514**, and `postmark-mcp` — the first malicious MCP server caught in the wild), CometJacking (agentic-browser injection), ServiceNow Now Assist agent-to-agent injection, and the **Semantic Kernel prompt-injection→RCE pair** (**CVE-2026-26030** / **CVE-2026-25592**, the framework's own `eval()` and file-write tool as the sink), and **IDEsaster** (a universal attack chain — **24 CVEs across Cursor, GitHub Copilot, Windsurf, Roo Code, Junie and more** — where a poisoned repo turns the AI IDE's own features into data-exfil/RCE), and the **LiteLLM AI-gateway breach** (**CVE-2026-42271** — CISA-KEV, exploited-in-the-wild command-injection→RCE in the LLM proxy, chaining via a Starlette Host-header bypass to unauthenticated **CVSS 10.0**), and the **PickleScan scanner-bypass trio** (**CVE-2025-10155 / -10156 / -10157**, all **CVSS 9.3** — malicious PyTorch models the model-file scanner rates clean but that still execute on load, because the scanner and the loader parse the file differently), and the **Claude Code trust-dialog bypass pair** (**CVE-2025-59536** RCE / **CVE-2026-21852** API-key exfiltration — a cloned repo's project config runs code and leaks your API key *before* the AI coding CLI's trust prompt fires, via Hooks / MCP config / a malicious `ANTHROPIC_BASE_URL`; fixed in `1.0.111` and `2.0.65`) — plus authoritative guidance (NSA AISC MCP CSI, OWASP Top 10 for LLMs 2025 & Agentic 2026, and the MITRE ATLAS agent-technique expansion). | **[INCIDENTS_AND_GUIDANCE_2026.md](INCIDENTS_AND_GUIDANCE_2026.md)** |
| 🧰 **AI Security Tooling Catalog** | The open-source tools practitioners actually run — garak, PyRIT, mcp-scan, LLM Guard, ModelScan, ART — organized by *what you're trying to do*, with honest maintenance status. | **[TOOLS.md](TOOLS.md)** |
...
| **[OWASP GenAI Security Solutions Landscapes (Q2 2026)](https://genai.owasp.org/resource/ai-security-solutions-landscape-for-ai-and-agentic-red-teaming-q2-2026/)** | OWASP | Q2 2026 trio mapping the **tooling ecosystem** onto the LLM/GenAI and **agentic** lifecycles — including OWASP's **first dedicated Red Teaming Landscape** and an agentic red-teaming taxonomy. The "what to defend with" companion to the Top 10's "what can go wrong." [Breakdown](STANDARDS_AND_COMPLIANCE.md#-owasp-genai-security-solutions-landscapes-q2-2026--including-the-first-red-teaming-landscape). |
