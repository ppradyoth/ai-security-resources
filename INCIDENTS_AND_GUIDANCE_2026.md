...
8. **Don't expect a guardrail to save you — measure it.** The peer-reviewed grounding under these incidents (see *[Agentic Coding Assistants & Tool/Protocol Exploitation](RESEARCH_PAPERS.md#-agentic-coding-assistants--toolprotocol-exploitation)* in the papers list) reports attack success rates up to **84%** against Cursor/Copilot and finds **most of 18 published defenses achieve <50% mitigation** against adaptive attacks. Treat prompt injection as an architecture-level problem (isolation, least privilege, human-in-the-loop on high-impact actions), not a filter you bolt on — and publish your guardrail's measured false-negative rate rather than the fact that it exists.
