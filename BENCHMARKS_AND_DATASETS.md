...
*   **Purpose**: A benchmark for the **harmfulness of LLM agents** — a collaboration between **Gray Swan AI** and the **UK AI Safety Institute** (core contributors from EPFL, CAIS, CMU, Oxford; ICLR 2025, [arXiv:2410.09024](https://arxiv.org/abs/2410.09024)). Ships **110 explicitly malicious agent tasks (440 with augmentations)** across **11 harm categories** (fraud, cybercrime, harassment, and more). The public dataset is **partially released** (44/66 public-test base behaviors + validation set) under an MIT license with a **safety-only-use** clause, to blunt misuse.
*   **Why it matters**: This is the *third leg* of the agentic-eval stool and measures a different axis than the two above. AgentDojo and InjecAgent test whether an agent can be **hijacked by an injection**; AgentHarm tests whether an agent will **directly comply with an openly malicious multi-step request** — and, critically, it scores not just refusal but whether a *jailbroken* agent **retains the capability to actually complete the harmful task**, since a model that "agrees" but then bungles the tool calls poses little real risk. It ships ready-to-run inside the UK AISI's **[Inspect](https://github.com/UKGovernmentBEIS/inspect_ai)** evals framework (already cataloged below), so a run is a `pip install` away.

#### [Agent Security Bench (ASB)](https://github.com/agiresearch/ASB)
*   **Purpose**: A framework that **formalizes and benchmarks both attacks *and* defenses** across the full LLM-agent operating surface (Rutgers/AGI Research; ICLR 2025, [arXiv:2410.02644](https://arxiv.org/abs/2410.02644)). Spans **10 scenarios** (e.g., e-commerce, finance, autonomous driving, legal/investment advising), **10 agents**, **400+ tools**, **27 attack/defense methods**, and **7 evaluation metrics**, run across **13 LLM backbones**.
*   **Why it matters**: Where AgentDojo/InjecAgent focus on the *injection* entry point, ASB maps the threat across **every stage of agent operation** — system prompt, user prompt, tool use, and memory retrieval — pairing **Direct** and **Indirect Prompt Injection** with a **Memory-Poisoning** attack and a novel **Plan-of-Thought (PoT) backdoor**, plus 4 mixed attacks and **11 corresponding defenses**. Its headline finding is the useful, sobering one for defenders: a **highest average attack success rate of 84.30%** with **current defenses showing limited effectiveness** — i.e., the benchmark exists precisely because the defensive side is still losing. Use it as the *attack-and-defense-coverage* leg alongside AgentDojo (injection utility), InjecAgent (injection taxonomy), and AgentHarm (direct harm compliance).
...
> **Link integrity:** Every repository link on this page is verified against its canonical source. Last full verification pass: **2026-06-30** — corrected the AgentDojo, garak, Inspect AI, Do-Not-Answer, HarmfulQA, FigStep, and `jailbreak_llms` links to their canonical repositories, and added InjecAgent. **2026-07-10** — added **AgentHarm** (ICLR 2025; Gray Swan AI × UK AISI), completing the agentic-eval trio (injection-hijack → indirect-injection → agent-harmfulness); links verified against arXiv, the AISI Hugging Face dataset, and the Inspect evals framework. **2026-07-11** — added **Agent Security Bench (ASB)** (ICLR 2025; [arXiv:2410.02644](https://arxiv.org/abs/2410.02644), [agiresearch/ASB](https://github.com/agiresearch/ASB)) as the attack-and-defense-coverage leg spanning the full agent lifecycle; verified against the arXiv record, OpenReview (ICLR 2025), and the official GitHub repo. Spot a stale or moved link? [Open an issue or PR](CONTRIBUTING.md).
>
